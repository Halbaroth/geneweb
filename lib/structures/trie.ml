...
module Make (W : Word.S) = struct
  ...
  type char_ = W.char_
  type word = W.t
  type 'a t = { next : 'a t M.t; data : 'a option; cardinal : int }

  let empty = { next = M.empty; data = None; cardinal = 0 }
  ...
  let update w f t =
    ...
    let rec loop t i =
      ...
      if i = len then
        ...
        let c = W.get w i in
        let child =
          match M.find c t.next with
          | exception Not_found -> empty
          | child -> child
        in
        let diff, nchild = loop child (i + 1) in
        let next =
          if is_empty nchild then M.remove c next
          else M.add c nchild next
        in
        (diff, { t with next; cardinal = t.cardinal + diff })
    in
    loop t 0 |> snd

  let add w v t = update w (fun _ -> Some v) t
  let remove w t = update w (fun _ -> None) t
  let next c t = M.find c t.next
