...
val src : Logs.src
(** The sources to which error and debug messages from the RPC server are
    reported. *)
...
val start :
  ...
  handler ->
  unit
(** [start ~interface ~port hdl] initializes an
    asynchronous RPC server based on a HTTP/1.1 server with WebSocket
    capability.
